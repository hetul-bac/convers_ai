...
type SettingsWorkspaceProps = {
  ...
  currentMonthSummary: BillingSummary;
  usageRows: UsageRow[];
};
...
export function SettingsWorkspace({
  ...
  const [activeTab, setActiveTab] =
    useState<(typeof tabs)[number]["id"]>("api_keys");
  ...
  const usagePercent =
    currentMonthSummary.limit > 0
      ? Math.min(
          100,
          Math.round(
            (currentMonthSummary.messages_used / currentMonthSummary.limit) * 100,
          ),
        )
      : 0;
  const totalApiCalls = usageRows.reduce((sum, row) => sum + row.call_count, 0);
  const averageLatency =
    usageRows.length > 0
      ? Math.round(
          usageRows.reduce((sum, row) => sum + row.avg_response_ms, 0) /
            usageRows.length,
        )
      : 0;
          ...
          return (
            <button
              ...
              className={`rounded-full px-5 py-3 text-sm font-semibold transition ${
                active
                  ? "bg-[#3182ce] text-white shadow-[0_14px_28px_rgba(49,130,206,0.24)]"
                  : "bg-white text-slate-700 shadow-[0_12px_24px_rgba(15,23,42,0.06)] hover:bg-slate-50"
              }`}
          ...
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            <article className="surface-panel p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="rounded-2xl bg-[#ebf5ff] p-3 text-[#3182ce]">
                  <CircleDollarSign className="h-5 w-5" />
                </div>
              </div>
              <p className="mt-8 text-3xl font-semibold text-slate-950">
                {currentMonthSummary.plan}
              </p>
            </article>
            ...
            <article className="surface-panel p-5">
              ...
              <p className="mt-8 text-3xl font-semibold text-slate-950">
                {currentMonthSummary.messages_used.toLocaleString()}
              </p>
            </article>
            ...
            <article className="surface-panel p-5">
              ...
              <p className="mt-8 text-3xl font-semibold text-slate-950">
                ${currentMonthSummary.cost_so_far.toFixed(2)}
              </p>
            </article>
            ...
            <article className="surface-panel p-5">
              ...
              <p className="mt-8 text-3xl font-semibold text-slate-950">
                {currentMonthSummary.limit.toLocaleString()}
              </p>
            </article>
          </section>
          ...
          <section className="surface-panel p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">
                  {currentMonthSummary.messages_used.toLocaleString()} of{" "}
                  {currentMonthSummary.limit.toLocaleString()} messages used this month.
                </p>
              </div>
            ...
            <div className="mt-6 h-4 overflow-hidden rounded-full bg-slate-100">
              <div
                className="h-full rounded-full bg-[linear-gradient(90deg,#3182ce_0%,#63b3ed_100%)]"
      ...
      {activeTab === "usage" ? (
        <div className="space-y-6">
          <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <article className="surface-panel p-5">
              <p className="text-sm text-slate-500">Tracked Endpoints</p>
              <p className="mt-8 text-3xl font-semibold text-slate-950">
                {usageRows.length}
              </p>
            </article>
              ...
              <table className="min-w-full">
                <thead className="bg-slate-50/80 text-left text-xs uppercase tracking-[0.2em] text-slate-500">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium">Call Count</th>
                  ...
                  {usageRows.length === 0 ? (
                    <tr>
                      <td className="px-6 py-10 text-center text-sm text-slate-500" colSpan={3}>
                        Usage logs will populate after API traffic starts.
                      </td>
                    </tr>
                  ) : (
                    usageRows.map((row) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">{row.call_count.toLocaleString()}</td>
                        <td className="px-6 py-4">{row.avg_response_ms} ms</td>
                      </tr>
                    ))
                  )}
                </tbody>
